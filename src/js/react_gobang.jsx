...
class GoBang extends React.Component {
    ...
    componentDidMount() {
        ...
        socket.on('play', function (data) {
            //更新视图
            const history = that.state.history.slice(0, that.state.stepNumber + 1);
            const current = history[history.length - 1];
            const squares = current.squares.slice();
            let val = that.state.isTurnBlack ? "1" : "2"; //1：黑棋 2：白棋
    ...
    componentDidUpdate() {
        ...
        let val = !this.state.isTurnBlack ? "1" : "2"; //1：黑棋 2：白棋
    ...
    render() {
        ...
        let role = this.state.meBlack ? 'b' : 'w';

        let btn_restart_disabled = false;
        let btn_withdraw_disabled = true;
        let btn_undowithdraw_disabled = true;
        ...
        if (this.state.history.length > this.state.stepNumber + 1) {
            let withrow_role = this.state.meBlack ? 'b' : 'w';
